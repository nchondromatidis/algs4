...
public class TrieST_EN<Value> {
    ...
    private Node get(Node x, String key, int d) {
        ...
        int c_int = (int)c;
        c_int -= 65;
        return get(x.next[c_int], key, d+1);
    }
    ...
    private Node put(Node x, String key, Value val, int d) {
        ...
        int c_int = (int)c;
        c_int -= 65;
        x.next[c_int] = put(x.next[c_int], key, val, d+1);
        return x;
    }
    ...
    public Boolean key_prefix_exists(String prefix) {
        Node x = get(root, prefix, 0);
        if (x == null) return false;
        return true;
    }

    public Node key_prefix_exists(Node x, String prefix, int d) {
        if (x == null) return null;
        if (d == prefix.length()) return x;
        char c = prefix.charAt(d);
        int c_int = (int)c;
        c_int -= 65;
        return get(x.next[c_int], prefix, d+1);
    }
}
